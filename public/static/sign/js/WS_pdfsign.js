...
function GetEsealPic(strCertID, cb, ctx) {
	if(bActiveX){
		if ($_$ClientLoginObj != null) {
			return $_$ClientLoginObj.ESeaL_GetStampPic(strCertID, cb, ctx);
		} else {
	        return $myErrorRtnFunc("", cb, ctx);
		}
	}else{
		if($_$CurrentObj != null){
			return $_$CurrentObj.GetEsealPic(strCertID, cb, ctx);
		} else {
			return $myErrorRtnFunc("", cb, ctx);
		}
	}
...
function CreateWebSocketObject() {
    ...
    o.load_websocket = function() {
        ...
        var ws_url;
        ws_url = "ws://127.0.0.1:21051/pdfsign/"; //非TLS/SSL
        //ws_url = "wss://127.0.0.1:21061/pdfsign/";
        try {
    ...
    o.callMethod = function(strMethodName, cb, ctx, returnType, argsArray) {
        ...
        sendArray['pdfsign_func_name'] = strMethodName;
        //get sessionid
    ...
    o.GetEsealPic = function(strCertID, cb, ctx) {
    	var paramArray = [strCertID];
    	o.callMethod('GetEsealPic', cb, ctx, 'string', paramArray);
    }
...
function CreateAppObject_PDFClientLogin() {	
	var bOK = $LoadControl("A9A51080-2FC6-40D0-9491-7E641F39C70D", "PDFClientLogin", null, null);
	if (!bOK) {
		return null;
	}	
...
(function() {
    ...
    if ($_$XTXAppObj != null) {
        ...
        //PDFClientLogin实例obj
    	$_$ClientLoginObj = CreateAppObject_PDFClientLogin();
    	if ($_$ClientLoginObj == null) {
   			$XTXAlert("加载印章控件失败!");
    	}
        return;
