    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix" v-cloak>
        <div style="text-align: center">
          <span class="app-majorunit-info-title">{{ getForm.yf_dwmc }}</span>
        </div>
...
export default {
  ...
  methods: {
    ...mapActions(["getHelperunitInfo"]),
    initData() {
      this.loading2 = true;
      this.getHelperunitInfo({
        data: this.getForm
      }).then(data => {
  ...
  beforeMount() {
    if (sessionStorage.getItem("fz_dwmc")) {
      this.$set(this.getForm, "yf_dwmc", sessionStorage.getItem("fz_dwmc"));
      this.initData();
    } else {
      this.$router.push("/home/majorunit/index");
    }
  },
  beforeDestroy() {
    sessionStorage.removeItem("fz_dwmc");
  }
};
</script>
