<template>
  <div class="app-internet-recruit-form" v-loading="loading">
    <el-form
    ...
    >
      ...
      <el-form-item label="办学层次：">
        <template v-if="proConfig.education">{{ proConfig.education[form.education] }}</template>
      </el-form-item>
      <el-form-item label="办学日期：">
        <span>{{ form.date1 }} 至 {{ form.date2 }}</span>
      </el-form-item>
      <el-form-item label="平台名称：">{{ form.platform }}</el-form-item>
      <el-form-item label="平台网址：">{{ form.url }}</el-form-item>
      <el-form-item label="平台简介：">{{ form.introduction }}</el-form-item>
      <el-form-item
        :label="`${new Date(form.date1).getFullYear()}年学费：`"
        :rules="[{required: !query['3']}]"
      >{{ form.tuition1 }} 元</el-form-item>
      <el-form-item
        :label="`${new Date(form.date1).getFullYear() + 1}年学费：`"
        :rules="[{required: !query['3']}]"
      >{{ form.tuition2 }} 元</el-form-item>
      <el-form-item
        :label="`${new Date(form.date1).getFullYear() + 2}年学费：`"
        :rules="[{required: !query['3']}]"
      >{{ form.tuition3 }} 元</el-form-item>
      <el-form-item label="招生章程附件：">
        <el-link
          v-if="form.frdb_file"
          type="primary"
          icon="el-icon-view"
...
export default {
  name: "app-internet-recruit-form",
  data() {
    return {
      loading: false,
