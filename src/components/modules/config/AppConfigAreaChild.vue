  ...
  >
    <el-card>
      <div slot="header" class="clearfix" v-cloak>
        <el-button-group>
          <el-button @click.native="$router.go(-1)" type="primary" icon="el-icon-arrow-left">返回</el-button>
        ...
        <span style="font-size: 20px;padding-left: 20px">{{ title }}</span>
        <el-tag size="mini" type="success">省级</el-tag>
      </div>
      <div class="config-area-child-main">
...
export default {
  ...
  beforeMount() {
    ...
    this.$set(this.getForm, "pid", ID);
    this.$set(this.form, "pid", ID);
    this.initData();
    const areaSelect = sessionStorage.getItem("areaSelect");
    if (areaSelect) {
      this.selectData = JSON.parse(areaSelect);
    }
    if (sessionStorage.getItem("areaTITLE")) {
      this.title = sessionStorage.getItem("areaTITLE");
    }
  },
  beforeDestroy() {
    sessionStorage.removeItem("areaTITLE");
    sessionStorage.removeItem("areaSelect");
  }
};
</script>
