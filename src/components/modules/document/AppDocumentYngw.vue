    ...
    <el-card class="box-card">
      ...
      <div class="news inform-main" v-cloak>
        <el-table :data="tableData" style="width: 100%">
          ...
          <el-table-column align="center" prop="title" label="主题词"></el-table-column>
          ...
          <el-table-column align="center" prop="type" label="会议类型">
            <template slot-scope="scope">
              <div
                v-if="proConfig.document_type"
              >{{ proConfig.document_type[scope.row.document_type] }}</div>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="name" label="拟稿"></el-table-column>
          <el-table-column align="center" prop="tel" label="电话"></el-table-column>
          <el-table-column align="center" label="发文日期">
                ...
                <template v-if="userInfo.role === 1">
                  <template v-if="userInfo.chuID_1">
                    ...
                    <template v-if="scope.row.status === 3">
                      ...
                      <!--<template v-if="scope.row.document_user.length">-->
...
export default {
  ...
  data() {
    return {
      ...
      // 下发
      dialogVisible_lssued: false,
      dialog_loading_lssued: false,
  ...
  methods: {
    ...
    handleLssued(val) {
      this.$router.push({
        ...
        query: {
          ...
          issue: val.document_user.length, // 下发人员数量（0可下发、>0已下发）
    ...
    handlePrint(val, sign) {
      // sign：true签章操作
      let query = {
        print: `1${this.randomNumPublic(10, 99)}${val.id}${this.randomNumPublic(
          0,
          9
        )}`
      };
