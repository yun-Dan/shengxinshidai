<template>
  <div class="app-schedule-meetingroom" v-loading="loading">
    <div class="app-schedule-index-search clearfix">
      <el-autocomplete
        size="small"
        style="width: 240px"
        v-model="getForm.keyword"
    ...
    <el-calendar v-model="getForm.date_str">
      <template slot="dateCell" slot-scope="{date, data}">
        <div class="calendar-content-box">
          ...
          <template v-if="new Date(getForm.date_str).getMonth() + 1 === data.day.split('-').slice(1, 2).join('-') - 0">
            <template v-if="scheduleData[data.day.split('-').slice(2).join('-') - 0]">
              <div v-for="(item, index) in scheduleData[data.day.split('-').slice(2).join('-') - 0]"
                :key="index" style="margin-bottom: 4px">
      ...
      <div v-loading="dialogLoading" style="margin-right: 30px">
        <el-form :model="form" class="submit-el-form" label-width="100px">
          <el-form-item label="活动类型：" :rules="[{required: true}]">
            <el-select v-if="this.proConfig.schedule_type" v-model="form.type" placeholder="选择活动类型">
              <el-option
                v-for="(item, key) in this.proConfig.schedule_type"
                :label="item"
                :value="key - 0"
                :key="key"
              ></el-option>
...
export default {
  ...
  created () {
    let rooms = this.proConfig.schedule_name
    let users = this.proConfig.user_nam
    if (rooms && users) {
      this.$set(
        this.getForm,
        'keyword',
        rooms['1'] || this.$options.data().getForm.keyword
      )
