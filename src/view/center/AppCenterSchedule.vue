    ...
    <div class="app-schedule-leader" v-loading="loading">
      <div class="app-schedule-index-search clearfix">
        <el-select
          ...
          <el-option key="处长及副处" label="处长及副处" value="处长"></el-option>
          <el-option
        ...
        <template slot="dateCell" slot-scope="{date, data}">
          <div class="calendar-content-box">
            ...
            <template v-if="new Date(getForm.date_str).getMonth() + 1 === data.day.split('-').slice(1, 2).join('-') - 0">
              ...
              <div v-if="userInfo.role === 1" class="calendar-content-btn-box">
...
export default {
  name: 'app-center-schedule',
  data () {
    return {
      loading: false,
      proConfig: {},
      getForm: {
        date_str: new Date().getTime(),
        user: '所有人'
      },
      scheduleData: [],
      current_day: '',
  ...
  created () {
    this.proConfig = JSON.parse(localStorage.getItem('pro_config')) || {}
